...
import type { FormattedChunkItem, EventType } from "@/types/rag-stream"
...
export function StreamViewer({ items }: StreamViewerProps) {
  ...
  const [selectedEvent, setSelectedEvent] = useState<EventType | "unknown" | null>(null)
  ...
  const filteredItems = useMemo(() => {
    return items.filter((item) => {
      if (selectedTool && item.metadata?.tool?.name !== selectedTool) return false
      if (selectedEvent && item.event !== selectedEvent) return false
      if (searchQuery) {
        const content =
          typeof item.content === "string" ? item.content.toLowerCase() : JSON.stringify(item.content).toLowerCase()
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-semibold">Stream Events ({filteredItems.length})</h3>
        </div>
        ...
        {filteredItems.length === 0 ? (
          ...
          filteredItems.map((item) => <StreamEventCard key={`${item.index}-${item.sequence}`} item={item} />)
        )}
      </div>
    </div>
  )
